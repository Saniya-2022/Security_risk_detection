...
const Dashboard = () => {
  ...
  const connectWebSocket = () => {
    ...
    ws.onopen = () => {
      console.log('✅ WebSocket Connected to UNSW-NB15 System');
      setWsConnected(true);
    };
    ...
    ws.onmessage = (event) => {
      ...
      if (message.type === 'alert') {
        ...
        console.log('🔗 Connected to UNSW-NB15 IDS');
      }
    };
  ...
  const fetchInitialData = async () => {
    try {
      ...
      // Fetch model info
      const modelRes = await fetch('http://localhost:8000/models/info');
    ...
    <div className="dashboard">
      ...
      <header className="dashboard-header">
        <div className="header-left">
          ...
          <div className="system-subtitle">Real-Time ML-Based Network Security Monitoring</div>
      ...
      {modelInfo.best_model && (
        <div className="model-info-banner">
          <span>🤖 Active Model: <strong>{modelInfo.best_model.toUpperCase()}</strong></span>
          <span>🎯 Accuracy: <strong>{(modelInfo.results?.[modelInfo.best_model]?.accuracy * 100).toFixed(2)}%</strong></span>
          <span>📊 Features: <strong>{modelInfo.feature_count}</strong></span>
          <span>🔬 Training Samples: <strong>{modelInfo.training_samples?.toLocaleString()}</strong></span>
        </div>
      )}
      ...
      <div className="main-content">
        ...
        <div className="alert-feed">
          ...
          <div className="feed-subtitle">ML-Powered Intrusion Detection</div>
              ...
              alerts.map((alert, index) => (
                ...
                >
                  <div className="alert-header">
                    ...
                    <span className="threat-type">{alert.attack_category || alert.threat_type?.toUpperCase()}</span>
                    <span 
                    ...
                    {alert.ml_confidence && (
                      <span className="ml-confidence">
                        ML: {(alert.ml_confidence * 100).toFixed(1)}%
                      </span>
                    )}
                  </div>
                  ...
                  <div className="alert-body">
                    <div className="alert-source-info">
                      ...
                      <div className="info-item">
                        <strong>Dest IP:</strong> {alert.dest_ip}
                      </div>
                      <div className="info-item">
                        <strong>Protocol:</strong> {alert.protocol}
                      </div>
                      <div className="info-item">
                        <strong>Ports:</strong> {alert.source_port} → {alert.dest_port}
                      </div>
                      <div className="info-item">
                        <strong>Detected By:</strong> {alert.detected_by}
                      </div>
                      <div className="info-item">
                        <strong>Method:</strong> {alert.detection_method}
                      </div>
                    </div>
              ...
              {stats.threat_distribution && stats.threat_distribution.length > 0 ? (
                stats.threat_distribution.map((threat, index) => (
                  <div key={index} className="threat-item">
                    <span className="threat-icon">{getThreatIcon(threat._id?.toLowerCase())}</span>
                ...
                stats.top_source_ips.map((ip, index) => (
                  <div key={index} className="ip-item">
                    ...
                    <div className="ip-details">
                      <div className="ip-address">{ip._id}</div>
                      <div className="ip-stats">
                        {ip.count} alerts • Risk: {ip.avg_risk?.toFixed(1)}
                      </div>
                    </div>
                  </div>
                ))
          ...
          <div className="widget">
            ...
            <div className="system-info">
              ...
              <div className="info-item">
                ...
                <span className={systemStatus.stream_service?.is_running ? 'status-ok' : 'status-error'}>
                  {systemStatus.stream_service?.is_running ? 'Running' : 'Stopped'}
                </span>
              </div>
              {systemStatus.stream_service?.progress && (
                <div className="info-item">
                  <span>Progress:</span>
                  <span>{systemStatus.stream_service.progress}</span>
                </div>
              )}
            </div>
          </div>
